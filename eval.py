...
def render_sets(model, net, opt, epoch: int):
    ...
    with torch.no_grad():
        ...
        with open(os.path.join(render_path, "results.txt"), "w") as f:
            for i in range(len(results)):
                psnr, ssim, lpips = results[i]['psnr'].item(), results[i]['ssim'].item(), results[i]['lpips'].item()
                f.write(f"{i} ---  PSNR: {psnr:.4f}, SSIM: {ssim:.4f},LPIPS: {lpips:.4f}\n")
