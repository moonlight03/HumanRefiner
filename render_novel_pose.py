...
def render_sets(model, net, opt, epoch:int,pose:str,stage:int, flag:str):
    with torch.no_grad():
        ...
        if pose == 'tpose':
            render_path = os.path.join(avatarmodel.model_path, 'novel_tpose_pose', "ours_tpose_{}_0{}".format(epoch, flag))
            novel_pose_dataset = avatarmodel.getNovelviewDataset(tpose=True)
            novel_pose_dataset.data_length = 100

        elif pose == 'observ':
            render_path = os.path.join(avatarmodel.model_path, 'novel_tpose_pose', "ours_observ_{}_0{}".format(epoch, flag))
            novel_pose_dataset = avatarmodel.getNovelviewDataset(0)
            novel_pose_dataset.data_length = 100
        elif pose == 'test':
        ...
        for idx, batch_data in enumerate(tqdm(novel_pose_loader, desc="Rendering progress")):
            ...
            if model.train_stage ==1:
                image, normal, points, _, _, _ = avatarmodel.train_stage1(batch_data, 59600, 0)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), network.extract(args), op.extract(args), args.epoch, args.pose, args.stage, args.flag)