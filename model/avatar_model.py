...
class AvatarModel:
    ...
    def load(self, iteration, test=False):
        ...
        #         sorted(os.listdir(join(r"data/m4c_processed/train", 'images')))):
        #     base_name = img.split('.')[0]
        #     name_list.append(base_name)
        #
        # name_list_ori = []
        # for index, img in enumerate(
        #         sorted(os.listdir(join(r"data/m4c_processed/train", 'images_ori')))):
        #     base_name = img.split('.')[0]
        #     name_list_ori.append(base_name)
        #
        # delete_list = []
        # save_list_index = []
        # for i, j in enumerate(name_list_ori):
        #     if j not in name_list:
        #         delete_list.append(j)
        #     else:
        #         save_list_index.append(i)
        ...
        if self.model_parms.train_stage == 1:
            # if not test:
            #
            #     self.pose.load_state_dict(saved_model_state["pose"]['weight'], strict=False)
    ...
    def getNovelviewDataset(self,tpose=False, t=0):
        if self.train:
            if tpose is True:
                self.novel_view_dataset = MonoDataset_tpose_novel_view(self.model_parms,self.train)
            else:
                self.novel_view_dataset = MonoDataset_novel_view(self.model_parms,self.train, t)
        else:
            if tpose is True:
    ...
    def train_stage1(self, batch_data, iteration, unseen_view):
        ...
        # a = torch.tensor([[-0.0906, 0.1335, 0.0190, -0.1041, -0.0995, 0.0505, 0.1448, -0.0354,
        #          -0.0381, 0.1311]], device='cuda:0')
        # b = torch.tensor([[-3.2163, 0.0354, -0.1827]], device='cuda:0')
        # c = torch.tensor([[-0.0075, 0.0830, 1.5]], device='cuda:0')  # f41.5 2.0   m41.5  di san wei
        #
        # d = torch.tensor([[0.0475, -0.0027, 0.0543, 0.0136, -0.1641, -0.0309, 0.1798, -0.0514,
        #          0.0178, 0.2101, -0.0660, 0.0350, 0.2135, -0.0475, -0.0252, 0.0109,
        #          -0.0401, -0.0455, -0.0641, 0.2574, -0.2694, -0.0782, -0.0953, 0.1159,
        #          0.0652, 0.1254, 0.0231, -0.1654, 0.1251, 0.4587, -0.0876, -0.1687,
        #          -0.3305, 0.0227, -0.1461, 0.0552, 0.1403, -0.1421, -0.1172, -0.1497,
        #          -0.0265, -0.0538, 0.1142, 0.1198, -0.0580, 0.1263, -0.0960, -0.9278,
        #          -0.0688, 0.4095, 0.5790, 0.2743, -0.7866, 0.1221, -0.3109, 1.6400,
        #          -0.7843, 0.8948, -0.1876, -0.1567, -0.0912, -0.6624, 0.2308, -0.0762,
        #          0.0871, -0.7677, 0.7438, 0.4963, 0.3834]], device='cuda:0')
        ...
        geom_featmap = self.geo_feature.expand(pose_batch.size(0), -1, -1, -1).contiguous()
        uv_coord_map = self.uv_coord_map.expand(pose_batch.size(0), -1, -1).contiguous()
        ...
        for batch_index in range(pose_batch.size(0)):
    ...
    def render_free_stage1(self, batch_data, iteration, degree):
        
        rendered_images = []
        pose_data = batch_data['pose_data']
        transl_data = batch_data['transl_data']

        c = torch.tensor([[0, degree, 0]], device='cuda:0')  # f41.5 2.0   m41.5  di san wei
