...
class MonoDataset_test(Dataset):
    ...
    def getitem(self, index, ignore_list):
        ...
        # index = [39,61,50][random.randint(0,3)]
        ...
        view = torch.from_numpy(np.array(view)) / 255.0
        resized_image = torch.from_numpy(np.array(image)) / 255.0
        if len(resized_image.shape) == 3:
            resized_image =  resized_image.permute(2, 0, 1)
            ...
            resized_image =  resized_image.unsqueeze(dim=-1).permute(2, 0, 1)
            view = view.unsqueeze(dim=-1).permute(2, 0, 1)
...
class MonoDataset_novel_view(Dataset):
    ...
    @torch.no_grad()
    def __init__(self, dataset_parms, train = False, t=0, device = torch.device('cuda:0')):
    ...
    def getitem(self, index,):
        ...
        if self.train is False:
            ...
            extr_npy =  self.get_freeview_camera(view_index[index], 100, Th)
            ...
            azimuth = -360.0 * (view_index[index] / 100.)
            if azimuth < -180.:
                azimuth +=360.
            ...
            data_item['view_index'] = view_index[index]
            data_item['FovX'] = FovX
            data_item['FovY'] = FovY
            data_item['width'] = width
            data_item['height'] = height
...
class MonoDataset_tpose_novel_view(Dataset):
    ...
    def getitem(self, index,):
        ...
        view = Image.open("data/dyn_m/train/view/back50.png").convert("RGB")
